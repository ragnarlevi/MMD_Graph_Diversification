...
def avg_degree(G):
    return np.mean([j for _, j in G.degree])

def plot_avg_degree(graphs:dict, rolling_window = 5, ax = None):
    """
    ...
    for i in range(nr_splits):
        avg_degree_split[i] = [avg_degree(graphs['graph_dict'][i][j]) for j in range(len(graphs['graph_dict'][i]))]
        ax.plot(graphs['dates'], pd.DataFrame(avg_degree_split[i]).rolling(rolling_window).mean().iloc[:,0], label = i, color = color_dark[i])

    ax.set_title(f'Average Degree for {graphs["sector"]}')
    ax.legend() 
